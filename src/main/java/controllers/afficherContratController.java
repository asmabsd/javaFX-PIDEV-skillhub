...
public class afficherContratController {
    ...
    void initialize() {
        try {
            ...
            this.filteredList = new FilteredList<>(this.observableList, contrat-> true); // Initialisation de filteredList
    ...
    FilteredList<Contrat> filteredList;
    @FXML
    ...
    @FXML
    void searchContrat(KeyEvent event) {
...
//itext for pdf done
    ...
    @FXML
    public void searchContrat(javafx.scene.input.KeyEvent keyEvent) {
        String searchText = searchBar.getText().toLowerCase();


        filteredList.setPredicate(contrat -> {
            if (searchText == null || searchText.isEmpty()) {
                return true;
            }
            return String.valueOf(contrat.getId()).contains(searchText)||
                    String.valueOf(contrat.getDate_debut()).contains(searchText)||
                    String.valueOf(contrat.getDate_fin()).contains(searchText)||
                    String.valueOf(contrat.getMontant()).contains(searchText)||
                    (contrat.getProjet()).contains(searchText)||
                    (contrat.getFreelancer()).contains(searchText)||
                    (contrat.getDescription()).contains(searchText)||
                    (contrat.getStatut()).contains(searchText);
        });
